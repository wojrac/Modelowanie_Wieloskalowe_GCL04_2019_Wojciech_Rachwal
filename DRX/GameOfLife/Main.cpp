...
int main()
{
	...
	RectangleShape circleShape3(Vector2f(a, b));
	circleShape3.setFillColor(Color::Black);
	Font font;
	...
	for (int i = 0; i <sizeY; i++)
	{
		...
		vector<long double> vectorZeroIdDis;
		vector<long double> vectorRightDownIdDis;
		vector<long double> vectorRightTopIdDis;
		vector<long double> vectorLeftTopIdDis;
		vector<long double> vectorWithDownRow1DDis;
		vector<long double> vectorWithTopRow1DDis;
		vector<long double> vectorWithLeftColumn1DDis;
		vector<long double> vectorWithRightColumn1DDis;
		vector<long double> vectorMiddle1DDis;
		vector<bool> vectorZeroIdRec;
		vector<bool> vectorRightDownIdRec;
		vector<bool> vectorRightTopIdRec;
		vector<bool> vectorLeftTopIdRec;
		vector<bool> vectorWithDownRow1DRec;
		vector<bool> vectorWithTopRow1DRec;
		vector<bool> vectorWithLeftColumn1DRec;
		vector<bool> vectorWithRightColumn1DRec;
		vector<bool> vectorMiddle1DRec;
		...
		double stepTime = 0;
		DRX drx;
		long double actuallDensity;
		long double tmpDensity;
		long double deltaRO;
		long double averageDis;
		long double equalityDivision;
		long double randomDivision;
		long double remainingEqualityDivision;
		long double onePackageDislocation;
		long double criticalDensitytmp;
		long double criticalDensity;
		long double deltaRoSum = 0;
		int cntZarodkowanie = 0;
		int cntVonNeuman = 0;
		vector<int> vectorWithCellsIdOnTheBorder;
		vector<int> vectorWithCellsIdOutTheBorder;
		std::fstream fileDensity;
		int cnt = 0;
		fileDensity.open("SumarycznyZapisGestosci.txt");
		...
		while (window.isOpen() )
		{
			...
			window.draw(buttonSizeMinusY);
					...
					//cout << sizeX;
			if (selectorPlay && isStarted &&MCiter<15)
			{
			...
			if (MCiter > 14 && stepTime <200)
			{
				...
				if (counterOfPA % 2 == 0)
				{
					...
					if (cnt == 0)
					{
						...
						for (int i = 0; i < sizeX*sizeY; i++)
						{
							myBoard[i].setCellIsOutTheBorder();
							//cout << myBoard[i].getCellColor() << endl;
						}
						for (int i = 0; i < 8; i++)
						{
						...
						for (int i = 0; i < 8; i++)
						{
							if (myBoard[0].getCellColor() != vectorZeroId[i])
								myBoard[0].setCellIsOnTheBorder();
							if (myBoard[rightTopId].getCellColor() != vectorRightTopId[i])
								myBoard[rightTopId].setCellIsOnTheBorder();
							if (myBoard[leftTopId].getCellColor() != vectorLeftTopId[i])
								myBoard[leftTopId].setCellIsOnTheBorder();
							if (myBoard[rightDownId].getCellColor() != vectorRightDownId[i])
								myBoard[rightDownId].setCellIsOnTheBorder();
						}
						for (int i = 1, j = 0; j < NumberOfCellInRow; i++, j++)
						{
							for (int k = 0; k < 8; k++)
							...
							for (int k = 0; k < 8; k++)
							{
								if (myBoard[i].getCellColor() != vectorWithDownRow1D[k])
									myBoard[i].setCellIsOnTheBorder();
							}
							vectorWithDownRow1D.clear();
						}
						for (int i = leftTopId + 1, j = 0; j < NumberOfCellInRow; i++, j++)
						{
							...
							for (int k = 0; k < 8; k++)
							{
								if (myBoard[i].getCellColor() != vectorWithTopRow1D[k])
									myBoard[i].setCellIsOnTheBorder();
							}
							vectorWithTopRow1D.clear();
						}
						for (int i = sizeX, j = 0; j < NumberOfCellInColumn; i += sizeX, j++)
						{
							...
							for (int k = 0; k < 8; k++)
							{
								if (myBoard[i].getCellColor() != vectorWithLeftColumn1D[k])
									myBoard[i].setCellIsOnTheBorder();
							}
							vectorWithLeftColumn1D.clear();
						}
						for (int i = rightDownId + sizeX, j = 0; j < NumberOfCellInColumn; i += sizeX, j++)
						{
							...
							for (int k = 0; k < 8; k++)
							{
								if (myBoard[i].getCellColor() != vectorWithRightColumn1D[k])
									myBoard[i].setCellIsOnTheBorder();
							}
							vectorWithRightColumn1D.clear();
						}
						for (int j = 0; j < (sizeX - 2)*(sizeY - 2); j++)
						{
							...
							for (int k = 0; k < 8; k++)
							{
								if (myBoard[temporaryArray[j]].getCellColor() != vectorMiddle1D[k])
									myBoard[temporaryArray[j]].setCellIsOnTheBorder();
							}
							vectorMiddle1D.clear();
						}
						vectorZeroId.clear();
						...
						for (int i = 0; i < sizeX*sizeY; i++)
						{
							if (myBoard[i].IsOnTheBorder())
								vectorWithCellsIdOnTheBorder.push_back(i);
							else
								vectorWithCellsIdOutTheBorder.push_back(i);
						}
				...
				for (int i = 0; i<sizeX*sizeY; i++)
				{
					myBoard[i].setTMPDensity(0);
				}
				//criticalDensitytmp = drx.countDensity(0.065);
				//criticalDensity = criticalDensitytmp /( sizeX*sizeY);
				//criticalDensity = 341223000;
				tmpDensity = drx.countDensity(stepTime / 1000);
				stepTime++; 
				//cout << stepTime << " ";  //zatrzymuje sie na 1193 iteracji
				actuallDensity=drx.countDensity(stepTime / 1000);   //wg excela przy stepTime==70 jest wartosc krytyczna, ale wtf
				fileDensity << actuallDensity << endl;
				deltaRO = actuallDensity - tmpDensity; 
				//if(stepTime==66)
				//deltaRoSum = deltaRO;
				averageDis = deltaRO / (sizeX*sizeY);
				equalityDivision = 0.3*averageDis;
				for (int i = 0; i < sizeX*sizeY; i++)
				{
					myBoard[i].setDensityOfDislocation(equalityDivision);
				}
				//int tmpTEST;
				onePackageDislocation = 0.7*averageDis * 0.01;
				int rDv; 
				int randNRCELL;
				for (int varToCnt = 0; varToCnt < 0.7*averageDis; varToCnt = varToCnt + onePackageDislocation)  //przechodzi przez wszystkie paczki w iteracji
				{		
					rDv = rand() % 100;
					if (rDv < 20)
					{
						
						randNRCELL = rand() % (vectorWithCellsIdOutTheBorder.size() - 1);
						
						myBoard[vectorWithCellsIdOutTheBorder[randNRCELL]].setDensityOfDislocation(onePackageDislocation);
					}
					else
					{
						randNRCELL = rand() % (vectorWithCellsIdOnTheBorder.size() - 1);
						//tmpTEST = randNRCELL;
						myBoard[vectorWithCellsIdOnTheBorder[randNRCELL]].setDensityOfDislocation(onePackageDislocation);
					}
				} //czyli cala ta petla nie miala znaczenia w kontekscie gestosci sumarycznej po podziale
				
				//cout<<myBoard[0].getDensityOfDislocation()<<endl;
				for (int i = 0; i < sizeX*sizeY; i++)
				{
					//deltaRoSum += myBoard[i].getTmpPackage();
				//deltaRoSum += myBoard[i].getDensityOfDislocation();    //za male i ida w dol
				//myBoard[i].setTmpPackage(0);
					deltaRoSum += myBoard[i].getTMPDensity();
				}
					for (int i = 0; i<sizeX*sizeY; i++)
					{
						myBoard[i].setDensityOfDislocation(myBoard[i].getTmpPackage());
					}
					for (int i = 0; i<sizeX*sizeY; i++)
					{
						myBoard[i].setTMPDensity(0);
					...
					/*for (int i = 0; i < sizeX*sizeY; i++)
					{
						deltaRoSum += myBoard[i].getDensityOfDislocation();     //za duze i ida w gore
						//myBoard[i].setTmpPackage(0);
					}*/
					//fileDensity << deltaRoSum<<endl;
					//deltaRoSum = 0;
					//criticalDensity = deltaRoSum /( sizeX*sizeY);//slabo,wszystkie ziarna na granicy zarodkuja
					//criticalDensity = myBoard[0].getDensityOfDislocation() + (sizeX*sizeY) *( 400000/(sizeX*sizeY));
					criticalDensity = drx.countDensity(0.065) / (sizeX*sizeY);
					for (int i = 0; i < sizeX*sizeY; i++)
					{
						//cout << myBoard[i].getTmpPackage() << endl;   //cos tam sie zmienia
						//co wazniejsze, po pierwszej iteracji nie bylo wzystkich kwaddratow na granicy czerwonych wiec
						//tu wchodzi
						if (myBoard[i].IsOnTheBorder() && myBoard[i].getDensityOfDislocation() > criticalDensity)
						{
							myBoard[i].setCellRecrystalized();
							myBoard[i].setCellRecrystalizedtmp();
							myBoard[i].setDensityOfDislocation(0);
							//cout << " if ";//ani razu nie wesz³o do ifa
						}
					}
						...
						for (int i = 0; i < sizeX*sizeY; i++)
						{
							if (myBoard[i].IsRecristalized())
							{
								myBoard[i].setCellColor(arrayOfStates[0]);
							}
						}
				...
				/*for (int i = 0; i < sizeX*sizeY; i++)
				{
					cout << myBoard[i].getDensityOfDislocation() << " ";
				}*/
				
				//tu chyba reguly przejscia trzeba zastosowac
				//if (cntZarodkowanie > 0)  //zaczynam  o jedno tempo pozniej niz zarodkowanie
			//	{
					//cout << endl;
					//cout << "ISRECRISTALIZEDtmpBEFORE" << endl;
					for (int i = 0; i < sizeX*sizeY; i++)
					{
						myBoard[i].setLessnestTrue();
						myBoard[i].setNeighAtLeastOneFalse();
						///cout << myBoard[i].IsRecristalizedtmp()<<" ";  //mimo sa jakies jedynki to potem nie wchodzi do ifa i tak
					}
					//cout << endl;
					if (counterOfPA % 2 == 0)
					{
						...
						//sprawdzam pierwsza zdolnosc czyli czy wszyscy sasiedzi sa mniejsi
						for (int i = 0; i < 4; i++)
						{

							vectorZeroIdDis.push_back(neighboursWithLeftDown[i].getDensityOfDislocation());
							vectorRightDownIdDis.push_back(neighboursWithRightDown[i].getDensityOfDislocation());
							vectorRightTopIdDis.push_back(neighboursWithRightTop[i].getDensityOfDislocation());
							vectorLeftTopIdDis.push_back(neighboursWithLeftTop[i].getDensityOfDislocation());

							vectorZeroIdRec.push_back(neighboursWithLeftDown[i].IsRecristalizedtmp());
							vectorRightDownIdRec.push_back(neighboursWithRightDown[i].IsRecristalizedtmp());
							vectorRightTopIdRec.push_back(neighboursWithRightTop[i].IsRecristalizedtmp());
							vectorLeftTopIdRec.push_back(neighboursWithLeftTop[i].IsRecristalizedtmp());

						}
						for (int i = 0; i < vectorZeroId.size(); i++)
						{
							if (myBoard[0].getDensityOfDislocation() < vectorZeroIdDis[i])
								myBoard[0].setLessnestFalse();
							if (myBoard[rightTopId].getDensityOfDislocation() < vectorRightTopIdDis[i])
								myBoard[rightTopId].setLessnestFalse();
							if (myBoard[leftTopId].getDensityOfDislocation() < vectorLeftTopIdDis[i])
								myBoard[leftTopId].setLessnestFalse();
							if (myBoard[rightDownId].getDensityOfDislocation() < vectorRightDownIdDis[i])
								myBoard[rightDownId].setLessnestFalse();

							if (vectorZeroIdRec[i])
								myBoard[0].setNeighAtLeastOneTrue();
							if (vectorRightTopIdRec[i])
								myBoard[rightTopId].setNeighAtLeastOneTrue();
							if (vectorLeftTopIdRec[i])
								myBoard[leftTopId].setNeighAtLeastOneTrue();
							if (vectorRightDownIdRec[i])
								myBoard[rightDownId].setNeighAtLeastOneTrue();
						}
						for (int i = 1, j = 0; j < NumberOfCellInRow; i++, j++)
						{
							for (int k = 0; k < 4; k++)
							{
								vectorWithDownRow1DDis.push_back(neighboursWithDownRow[j][k].getDensityOfDislocation());
								vectorWithDownRow1DRec.push_back(neighboursWithDownRow[j][k].IsRecristalizedtmp());
							}

							for (int k = 0; k < 4; k++)
							{
								if (myBoard[i].getDensityOfDislocation() < vectorWithDownRow1DDis[k])
									myBoard[i].setLessnestFalse();
								if (vectorWithDownRow1DRec[k])
									myBoard[i].setNeighAtLeastOneTrue();
							}
							vectorWithDownRow1DDis.clear();
							vectorWithDownRow1DRec.clear();
						}
						for (int i = leftTopId + 1, j = 0; j < NumberOfCellInRow; i++, j++)
						{
							for (int k = 0; k < 4; k++)
							{
								vectorWithTopRow1DDis.push_back(neighboursWithTopRow[j][k].getDensityOfDislocation());
								vectorWithTopRow1DRec.push_back(neighboursWithTopRow[j][k].IsRecristalizedtmp());
							}
							for (int k = 0; k < 4; k++)
							{
								if (myBoard[i].getDensityOfDislocation() < vectorWithTopRow1DDis[k])
									myBoard[i].setLessnestFalse();
								if (vectorWithTopRow1DRec[k])
									myBoard[i].setNeighAtLeastOneTrue();
							}
							vectorWithTopRow1DDis.clear();
							vectorWithTopRow1DRec.clear();
						}
						for (int i = sizeX, j = 0; j < NumberOfCellInColumn; i += sizeX, j++)
						{
							for (int k = 0; k < 4; k++)
							{
								vectorWithLeftColumn1DDis.push_back(neighboursWithLeftColumn[j][k].getDensityOfDislocation());
								vectorWithLeftColumn1DRec.push_back(neighboursWithLeftColumn[j][k].IsRecristalizedtmp());
							}
							for (int k = 0; k < 4; k++)
							{
								if (myBoard[i].getDensityOfDislocation() < vectorWithLeftColumn1DDis[k])
									myBoard[i].setLessnestFalse();
								if (vectorWithLeftColumn1DRec[k])
									myBoard[i].setNeighAtLeastOneTrue();
							}
							vectorWithLeftColumn1DDis.clear();
							vectorWithLeftColumn1DRec.clear();
						}
						for (int i = rightDownId + sizeX, j = 0; j < NumberOfCellInColumn; i += sizeX, j++)
						{
							for (int k = 0; k < 4; k++)
							{
								vectorWithRightColumn1DDis.push_back(neighboursWithRightColumn[j][k].getDensityOfDislocation());
								vectorWithRightColumn1DRec.push_back(neighboursWithRightColumn[j][k].IsRecristalizedtmp());
							}
							for (int k = 0; k < 4; k++)
							{
								if (myBoard[i].getDensityOfDislocation() < vectorWithRightColumn1DDis[k])
									myBoard[i].setLessnestFalse();
								if (vectorWithRightColumn1DRec[k])
									myBoard[i].setNeighAtLeastOneTrue();
							}
							vectorWithRightColumn1DDis.clear();
							vectorWithRightColumn1DRec.clear();
						}
						for (int j = 0; j < (sizeX - 2)*(sizeY - 2); j++)
						{
							for (int k = 0; k < 4; k++)
							{
								vectorMiddle1DDis.push_back(neighboursMiddle[j][k].getDensityOfDislocation());
								vectorMiddle1DRec.push_back(neighboursMiddle[j][k].IsRecristalizedtmp());
							}
							for (int k = 0; k < 4; k++)
							{
								if (myBoard[temporaryArray[j]].getDensityOfDislocation() < vectorMiddle1DDis[k])
									myBoard[temporaryArray[j]].setLessnestFalse();
								if (vectorMiddle1DRec[k])
									myBoard[temporaryArray[j]].setNeighAtLeastOneTrue();
							}
							vectorMiddle1DDis.clear();
							vectorMiddle1DRec.clear();
						}
						vectorZeroIdDis.clear();
						vectorRightDownIdDis.clear();
						vectorLeftTopIdDis.clear();
						vectorRightTopIdDis.clear();
						vectorZeroIdRec.clear();
						vectorRightDownIdRec.clear();
						vectorLeftTopIdRec.clear();
						vectorRightTopIdRec.clear();
					...
					for (int i = 0; i < sizeX*sizeY; i++)
					{
						myBoard[i].setCellUnRecrystalizedtmp();    ///to jest po to zeby bralo sie pod uwage tylko te z poprzedniego kroku czasowego
					}  //trzeba szukac czy bez tego jakkolwiek sie rozrasta!!!
					//cout << "DIV";
					/*cout << " ISATLEASTONEECNEIGH " << endl;
					for (int i = 0; i < sizeX*sizeY; i++)
					{
						cout << myBoard[i].IsAtLeastOneRecNeigh() << " ";     tu widac bylo ze jest wiecej niz w tmp
					}
					cout << endl;*/
					/*for (int i = 0; i < sizeX*sizeY; i++)
					{
						cout << myBoard[i].IsAllLesser() << " ";
					}*/
					////// mamy na gorze wyznaczone dla kazdej komorki ktora ma wszystkich sasiadow mniejszych a ktora nie 
					//cout << cntZarodkowanie << " ";
					//cout << endl;
					for (int i = 0; i < sizeX*sizeY; i++)
					{
						if (myBoard[i].IsAllLesser() && myBoard[i].IsAtLeastOneRecNeigh())
						{
							//cout << "if"; 
							myBoard[i].setCellRecrystalized();
							myBoard[i].setCellRecrystalizedtmp();
					...
					/*cout << endl;
					cout << "RECRISTALIZED@AFTRER" << endl;
					for (int i = 0; i < sizeX*sizeY; i++)
					{
						cout<<myBoard[i].IsRecristalizedtmp() << " ";
					}*/
				...
				/*for (int i = 0; i < sizeX*sizeY; i++)
				{
					...
					if (myBoard[i].IsRecristalizedtmp())
					{
						//cout << licznik << " ";   //tu wchodza takie same komorki, mimo ze nizej wchodzi dwa razy to i tak to sie nie zmienia
						myBoard[i].setCellColor(arrayOfStates[1]);						
						licznik++;
					}
				}*/
				//cntZarodkowanie++;
			...
			while (window.pollEvent(event))
			{
				...
				if (kt<0.1 || kt>6.0)
					kt = 3;		
				//byc moze tu  moglbym wyliczac ten wzor i sume zapisywac o pliku
				if (event.type == Event::Closed)
				{
					window.close();
				}
				...
				if (event.type == Event::MouseButtonPressed)
				{
					...
					if (Mouse::getPosition(window).x > ButtonX && Mouse::getPosition(window).x < (windowWidth-buttonWidth) && Mouse::getPosition(window).y >100 && Mouse::getPosition(window).y < (100 + buttonHeight) && !isStarted &&  tmpTFColumn.size()>0 && tmpTFRow.size()>0)
					{
						...
						else
						{
							...
							{
								arrayOfShapes[i].setFillColor(Color::Color(255 - i * 40, (i + 1) * 20, (i + 1) * 17));
					...
					if (Mouse::getPosition(window).x > ButtonX && Mouse::getPosition(window).x < (windowWidth - buttonWidth) && Mouse::getPosition(window).y >600 && Mouse::getPosition(window).y < (600 + buttonHeight) && selectorPlay)
					{
						...
						if (Mouse::getPosition(window).x > ButtonX && Mouse::getPosition(window).x < (windowWidth -buttonWidth) && Mouse::getPosition(window).y >500 && Mouse::getPosition(window).y < (500 + buttonHeight))
						{
							for (int i = 0; i < sizeX*sizeY; i++)
							{
								...
								myBoard[i].setCellUnRecrystalized();
								myBoard[i].setCellUnRecrystalizedtmp();
								myBoard[i].setTmpPackage(0);
								myBoard[i].setDensityOfDislocation(0);
							}
							isStarted = false;
